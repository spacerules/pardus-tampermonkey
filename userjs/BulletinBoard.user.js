...
// @version      1.0
// @description  Show multi-sector AP path info for missions on bulletin board with proper loading and sorting
...
(function() {
    ...
    function refreshHidden() {
        missionlist.forEach(item => {
            ...
            if (item.type === "transport") checkboxHide = !item.checkboxtransport;
            else if (item.type === "attack") checkboxHide = !item.checkboxAttack;
            else checkboxHide = !item.checkboxOther;

            const jumpHide = item.jumpfilter === false;
            const shouldHide = checkboxHide || jumpHide;

            item.mission.hidden = shouldHide;
            if (item.mission.nextElementSibling) item.mission.nextElementSibling.hidden = shouldHide;
        });
    }
    ...
    function setupFilters() {
        ...
        newbr1.insertAdjacentElement("afterend", newspan);

        const missionSettings = getGM_list();
        jumpLimit = missionSettings.jumpfilter;
    ...
    async function processMissions() {
        ...
        const promises = tables.map(async table => {
            ...
            if(!sectorMatch || !coordMatch){
                statusSpan.remove();
                completed++;
                if(progressDiv) progressDiv.textContent = `Loading missions: ${completed}/${total}`;
        ...
        // Append tables with single BR between
        ...
        // Update the span with jump/AP info
        resultsCache.forEach(r => {
            if(!r.span) return;
            if(r.result){
                ...
                r.span.textContent = "";
                r.span.appendChild(link);
            } else {
                r.span.textContent = "Error";
            }
        });
